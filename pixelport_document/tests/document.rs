...
fn test_remove_entity() {
    let mut doc = Document::from_string(PonRuntime::new(), r#"<Root><Entity name="tmp" x="5.0" /></Root>"#).unwrap();
...
fn test_property_get() {
    let mut doc = Document::from_string(PonRuntime::new(), r#"<Entity name="tmp" x="5.0" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
...
fn test_property_reference_straight() {
    let mut doc = Document::from_string(PonRuntime::new(), r#"<Entity name="tmp" x="5.0" y="@this.x" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
...
fn test_property_reference_object() {
    ...
    let mut doc = Document::from_string(runtime, r#"<Entity name="tmp" x="5.0" y="testy { some: @this.x }" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
...
fn test_property_reference_transfer() {
    let mut runtime = PonRuntime::new();
    runtime.register_function("something", |arg, runtime, doc| {
        let x = runtime.translate::<f32>(arg, doc).unwrap();
        Ok(Box::new(x * 2.0))
    }, "f32");
    let mut doc = Document::from_string(runtime, r#"<Entity name="tmp" x="5.0" y="something @this.x" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
    assert_eq!(doc.get_property::<f32>(ent, "y").unwrap(), 10.0);
}
...
fn test_property_reference_array() {
    ...
    let mut doc = Document::from_string(runtime, r#"<Entity name="tmp" x="5.0" y="testy [@this.x]" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
    assert_eq!(doc.get_property::<f32>(ent, "y").unwrap(), 10.0);
}
...
fn test_property_reference_parent() {
    let mut doc = Document::from_string(PonRuntime::new(), r#"<Entity x="5.0"><Entity name="tmp" y="@parent.x" /></Entity>"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
    assert_eq!(doc.get_property::<f32>(ent, "y").unwrap(), 5.0);
}
...
fn test_property_reference_not_yet_created() {
    let mut doc = Document::from_string(PonRuntime::new(), r#"<Entity name="tmp" y="@this.x" />"#).unwrap();
    let ent = doc.get_entity_by_name("tmp").unwrap();
    {
        doc.set_property(ent, "x", Pon::Number(9.0), false).ok().unwrap();
    }
...
fn test_document_to_string_empty() {
    let doc = Document::new(PonRuntime::new());
